...
- Mirror touch performance notes:
  - Root mirror service name is currently `wtmap_mirror_v5`; bump this name when root service behavior changes so old `app_process` helpers do not keep serving stale code after APK reinstall.
  ...
  - The upper screen runs the game through an emulator/container, so lower-screen aim dragging should be injected as mouse input: `SOURCE_MOUSE` with primary button state, not `SOURCE_TOUCHSCREEN`.
