...
- Mirror touch performance notes:
  - Root mirror service name is currently `wtmap_mirror_v6`; bump this name when root service behavior changes so old `app_process` helpers do not keep serving stale code after APK reinstall.
  ...
  - v6 root service writes input_event structs directly to that mouse device: `BTN_MOUSE` down, `REL_RX`/`REL_RY` deltas during drag, then `BTN_MOUSE` up. It falls back to Android `InputManager` injection if opening/writing the device fails.
...
- Tapping action buttons while the bottom button strip is expanded should not collapse the strip; preserve the expanded state across button actions, Activity recreation, and mirror-mode UI rebuilds. Only the bottom-left `>` control should change the collapsed/expanded state.
