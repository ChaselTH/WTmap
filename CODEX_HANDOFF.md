...
- 瞄准镜像触控优化：root 镜像服务已升级到 `wtmap_mirror_v6`。上屏游戏运行在模拟器/容器里，实际需要鼠标拖动；因此下屏拖动不再只走 Android `SOURCE_TOUCHSCREEN`/`SOURCE_MOUSE` 抽象事件，而是优先由 root 服务直接写 Thor 的 `ODIN Station Virtual Mouse`（通常 `/dev/input/event10`）：左键按下、REL_RX/REL_RY 相对移动、左键松开。MOVE 仍然 one-way Binder 发送并节流到约 60fps。
